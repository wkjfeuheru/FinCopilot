...
class MemoryStore(SqliteStore):
    ...
    def __init__(self, db_path: str | Path) -> None:
        super().__init__(db_path, check_same_thread=False, foreign_keys=True)
    ...
    def append_messages(self, conversation_id: str, messages: list[Msg]) -> tuple[int, int]:
        ...
        if not messages:
            return (0, 0)
        with self._connect() as connection:
            return self._append_messages(connection, conversation_id, messages)
    ...
    def save_conclusion(
        ...
        with self._connect() as connection:
            ...
            connection.execute(
                ...
                (user_id, conversation_id, subject, text, json.dumps(list(cids or []), ensure_ascii=False), utc_now_iso()),
            )
    ...
    def _fa_uid(user_id: str, key: str) -> str:
        ...
        digest = hashlib.sha256(f"{user_id}|{key}".encode()).hexdigest()[:12]
        return f"fa_{digest}"
