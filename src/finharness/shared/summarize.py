"""长文档的 map-reduce 摘要内核（docs 03.10）。

为什么不是"把全文塞给一次模型调用"：单篇研报、公告或报告主体动辄数万字，远超一次
请求的窗口，也远超单条工具结果的预算。因此把它拆成能各自容纳的分片，分别摘要，
再归并——这就是 map/reduce。

这里只放**纯逻辑**：分片、骨架、id 与乱序复原、归并分组。真正的 LLM 调用由
``summarize_document`` 工具通过会话的 ``Coordinator.spawn`` 驱动（复用既有子代理，
不新造一条调用路径）。

两条必须正视的固有代价，本模块各有一条对策：

* **并行结果会乱序。** ``Coordinator.spawn`` 本身保序，但 map 是并发、reduce 是
  LLM、且分片数超过一批时还要分批，因此顺序不能作为契约。对策是给每片发布稳定
  id（``P01``…），要求每片摘要以自身 id 开头，归并前**按 id 重排**——顺序由数据
  决定，而不由返回次序决定。
