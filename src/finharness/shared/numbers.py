...
摘要会在两处丢数字：分片结论被裁剪（截断），或汇合时取舍掉具体值。前者由调用方
留下的裁剪标记留下确证痕迹，后者常留下一个悬空的量词（如"同比少增…"）。

本模块只做**纯逻辑**：识别缺口、给出定位锚文本、由锚文本回到原文页码。
``summarize_document`` **不再**自动派子代理补齐；主 Agent 可按标注用 ``read_pdf``
或按片 ``spawn_agent`` 自行决定是否精读。

为什么只查"缺值"而不比对原文数字全集：一份研报有数百个数字，摘要本就不可能全收，
按全集比对会把正常的取舍成批误报为缺失。可判定为缺陷的是
...
# 分片结论被裁剪时留下的标记。调用方可把它当作截断后缀；本模块据此还原成一处缺口。
CLIP_MARKER = "…（本片摘要已截断）"
...
# 一次核对最多识别的缺口数：封顶下游精读工作量，避免异常输入导致膨胀。
MAX_GAPS = 12
