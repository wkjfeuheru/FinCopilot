...
**时效必须给出证明**：工具结果顶部的「数据时效」段落列出各序列的数据期、发布机构与发布节奏，
直接采用。用户问「最新／当前」时，回答须让读者无需自行判断就能确认时效：给出数据期、发布机构，
并说明该期是否即当前最新已发布期（数据期落在当前月份之前属正常，月度指标按月发布）；
用户指名尚未发布的期时，如实说明该期未发布及预计发布日，不得编造。判读方法见
references/indicators.md 的「发布节奏速查」。
