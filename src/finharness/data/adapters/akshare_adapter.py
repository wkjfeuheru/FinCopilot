...
class AkShareAdapter(DataAdapter):
    ...
    def _call(self, interface: str, build: Callable[[Any], pd.DataFrame]) -> pd.DataFrame:
    ...
    def fetch_quote(self, symbol: str) -> FetchResult:
        ...
        errors: list[str] = []
        for interface in AKSHARE_ENDPOINTS["quote"]:
            try:
                if interface == "stock_zh_a_spot_em":
                    df = _normalize(self._call(interface, lambda ak: ak.stock_zh_a_spot_em()), "quote")
                    if "symbol" not in df.columns:
                        raise AdapterError(f"{interface}: 缺少代码列")
                    hit = df[df["symbol"].astype(str).str.zfill(6) == symbol]
                elif interface == "stock_zh_a_spot":
                    raw = self._call(interface, lambda ak: ak.stock_zh_a_spot())
                    df = raw.rename(
    ...
    def fetch_indicators(self, symbol: str, years: int, fields: list[str] | None) -> FetchResult:
        ...
        if "日期" in df.columns:
            ...
            df = df.sort_values("date", ascending=False)
        if fields:
            keep = [c for c in df.columns if c == "date" or any(f in str(c) for f in fields)]
            if keep:
                df = df[keep]
        return FetchResult(df=df.reset_index(drop=True), interface=interface)
    ...
    def fetch_valuation(self, symbol: str, lookback_years: int) -> FetchResult:
        ...
        df = self._call(
            ...
            lambda ak: ak.stock_zh_valuation_baidu(
                symbol=symbol, indicator="总市值", period=period
            ),
        )
        if "date" in df.columns:
            df["date"] = pd.to_datetime(df["date"], errors="coerce")
