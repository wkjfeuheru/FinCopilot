...
6. **并行子代理**：`spawn_agent` 把若干彼此独立的并行任务分派给子代理，每个子代理在**自己的上下文**里完成、只回结论，因此大量中间材料（多份长文档、逐实体深分析）不占主上下文。触发扇出时主 Agent 须先**逐单元拆解**任务、子代理各自执行后主 Agent 再**汇合**成统一结论。子代理只读；模型可见的 `general` 子代理在任务点名标的时**可自行取数**（内部 `reader` 焦点只读材料、不取数，供长文摘要用）。
...
| GET | `/v1/trace/runs/{run_id}` | 完整 trace：每步 FSM `states` + 逐轮思考 / 工具往返 / 事件（含跑偏点） |
| GET | `/v1/trace/metrics` | 指标聚合（完成率 / 调用数 / 步数 / 状态步数 / 失败率 / 重复率 / 拦截 / 超时） |
...
- **多智能体做两件事：风险终审 + 通用任务扇出**，共同理由是**上下文隔离**（不是并发）。
  ...
  已在第二个焦点出现后按原定条件落地；模型可见的 `general` 子代理**可自行取数**（任务点名
  标的时），内部 `reader` 焦点只读材料、不取数（长文摘要用），`risk` 终审独立核数。
  决策记录见 [03.10-coordinator.md](docs/modules/03.10-coordinator.md)。
